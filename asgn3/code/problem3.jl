...
function grad_F(X::Array{Float64,2}, sigma::Float64, alpha::Float64)
	alpha_helper = (x,s,a)->d_alpha_grad_studentt(x,s,a).*(studentt(x,s,a).^-1)-grad_studentt(x,s,a).*(studentt(x,s,a).^-2).*d_alpha_studentt(x,s,a)
	sigma_helper = (x,s,a)->d_sigma_grad_studentt(x,s,a).*(studentt(x,s,a).^-1)-grad_studentt(x,s,a).*(studentt(x,s,a).^-2).*d_sigma_studentt(x,s,a)
	...
	return dsigma::Array{Float64,2}, dalpha::Array{Float64,2}
end
...
function prediction(X::Array{Float64,2}, Y::Array{Float64,2}, sigma_noise::Float64,
					...
					dsigma, dalpha = grad_F(X, sigma, alpha)
...
function problem3()
  ...
  imshow(img_noisy, "gray")
	...
	display(theta)
	#img_denoised = denoise(img_noisy, sigma_noise, theta[1], theta[2])
  #PyPlot.figure()
